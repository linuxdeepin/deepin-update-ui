...
void FullScreenBackground::exit()
{
    this->close();
}
...
void FullScreenBackground::setContent(QWidget *const w)
{
    ...
    qInfo() << "current frame size: width " << currentFrame->width() << " height " << currentFrame->height();
...
void FullScreenBackground::paintEvent(QPaintEvent *e)
{
    ...
    if (m_useSolidBackground) {
        ...
        QPixmap &blurBackground = getPixmap(PIXMAP_TYPE_BLUR_BACKGROUND);
        blurBackground.setDevicePixelRatio(devicePixelRatioF());
...
void FullScreenBackground::resizeEvent(QResizeEvent *event)
{
    ...
    if (currentFrame == this && currentContent) {
        qInfo() << "resize event: width " << size().width() << " height " << size().height();
...
QPixmap& FullScreenBackground::getPixmap(int type)
{
